...
def seperate():
    ...
    train_word = [] # shape: relation_num*len(i)*fixlen, relation_num是指所有tuple的关系数
    train_pos1 = []
    train_pos2 = []

    print('seprating train data')
    # 一个特定的i对应一个tuple的一个关系标签
    for i in range(len(x_train)):
        word = []   # shape: len(i)*fixlen, len(i)是指一个tuple的一个关系标签中训练句子的个数
        pos1 = []
        pos2 = []
        # 一个特定的j对应一个tuple的特定关系标签的一个训练句子
        for j in x_train[i]:
            temp_word = []  #shape: 1*fixlen
            temp_pos1 = []
            temp_pos2 = []
            for k in j:
