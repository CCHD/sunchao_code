...
import os
import network
from sklearn.metrics import average_precision_score, f1_score, confusion_matrix, precision_score, recall_score, precision_recall_fscore_support
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # todo
            testset = []
            with open("./origin_data/test.txt", encoding='utf8') as f:
                for line in f:
                    testset.append(line)
            ...
            for line in testset:
                ...
                prob, accuracy, feature_map = test_step(test_word, test_pos1, test_pos2, test_y)
                ...
                y_pred.append(top3_id[0])
                y_true.append(relation2id[relation])
            print(y_true)
            print(y_pred)
            print(confusion_matrix(y_true, y_pred))
            precision = precision_score(y_pred, y_true, labels=[0, 1, 2, 3, 4], average="weighted")
            recall = recall_score(y_true, y_pred, labels=[0, 1, 2, 3, 4], average='weighted')
            f1 = f1_score(y_true, y_pred, labels=[0, 1, 2, 3, 4], average='weighted')
            f1_ = 2 * precision * recall / (precision + recall)
            print("{:.1f}\t{:.1f}\t{:.1f}".format(100 * precision, 100 * recall, 100 * f1_))
            # print(100 * recall)
            # # print(100 * f1)
            # print(100 * f1_)
